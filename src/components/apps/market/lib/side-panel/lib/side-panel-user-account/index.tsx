...
const SidePanelUserAccount: React.FC<Props> = ({ userProfile }) => {
      ...
      <section className="flex gap-3 items-center">
        ...
        <div className="flex flex-col gap-1 justify-center">
          <span className="font-semibold text-[16px]" style={{ lineHeight: '12px' }}>
            {userProfile?.businessName ?? userProfile?.userName ?? `Business`}{' '}
          </span>
          <span className="post-status text-[14px] font-light">{userProfile?.name}</span>
        </div>
      </section>
      <section>
        <Popover
...
export default SidePanelUserAccount;
